...
- [x] Ephemeral messages — `/secret <msg>` / `/s <msg>` sets `MessageAd.ephemeral` (AAD-bound). Server skips join-replay history when set; receivers' TUIs render as `EPHEMERAL_MASK = "*******"`, auto-delete after `EPHEMERAL_TTL_MS = 30_000`, Ctrl-R reveals briefly (`EPHEMERAL_REVEAL_MS = 3_000`). Client gets a 500 ms tick in `event_loop` to drive expiry. 2 new unit tests + 1 e2e (`ephemeral_message_broadcasts_but_not_in_history`). BREAKING wire change → `PROTOCOL_VERSION` 1 → 2 (postcard is positional; field add is wire-incompatible).
